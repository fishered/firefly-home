...
::: warning 未单独发布
v1.0.5 实现已经通过 [Firefly PR #26](https://github.com/fishered/Firefly/pull/26) 合并到 `master`，但没有创建 `v1.0.5` 源码 tag，也没有单独发布 Maven Central 构件。这些能力随后随 `v1.0.6` 正式交付，请直接使用 `1.0.6`，不要声明 `1.0.5` 依赖。
:::

## 非 Spring Java Remote Adapter

该能力首次随正式版本提供的 Maven Central 构件：
...
正式发布的 `io.github.fishered:firefly-bom:1.0.6` 可以在 Maven 项目中只固定一次 Firefly 版本：
