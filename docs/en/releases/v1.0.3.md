...
The `v1.0.3` source tag exists, but the Maven Central publication workflow has not run yet. Quick Start, Integration, and Maven Central pages therefore continue to reference the published `1.0.2` artifacts. Upgrade only after `1.0.3` is visible in Central Portal.
:::
...
1. Confirm that `1.0.3` is published and indexed in [Central Portal](https://central.sonatype.com/artifact/io.github.fishered/firefly-spring-boot-starter); remain on `1.0.2` until then.
