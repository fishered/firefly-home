---
title: Maven Central
description: Firefly 1.0.0 的 Maven Central 坐标、模块依赖链和版本策略。
---

# Maven Central

Firefly `1.0.0` 已通过 `io.github.fishered` 命名空间发布到 Maven Central。Spring Boot 业务项目只需要引入一个 Starter，无需添加私有仓库或单独声明 Firefly 的内部模块。

[在 Maven Central 查看 firefly-spring-boot-starter 1.0.0](https://central.sonatype.com/artifact/io.github.fishered/firefly-spring-boot-starter/1.0.0)
...
正常情况下可以看到 `firefly-spring-boot-starter`、`firefly-spring-boot-autoconfigure`、`executor-netty`、`netty` 和 `scheduler-core`。

## 版本策略

Maven Central 中的正式版本不可覆盖。项目升级时应显式修改版本号；`1.0.0` 发布后的修复将使用新的补丁版本，例如 `1.0.1`。
